...
class GeminiService {
      ...
      try {
        ...
        const errorMessage =
          importError instanceof Error
            ...
            : String(importError);
        logger.warn(
          `pdf-lib not available for PDF page limiting: ${errorMessage}. Install with: npm install pdf-lib`
        );
        logger.info(
  ...
  async extractInvoiceData(
    ...
    try {
      logger.info(`Starting Gemini extraction for ${documentType}: ${s3Key}`);
      ...
      try {
        let fileBuffer = await S3Service.getFileBuffer(s3Key);

        const keyLower = s3Key.toLowerCase();
        if (keyLower.includes(".pdf") || keyLower.endsWith(".pdf")) {
        ...
        } else if (keyLower.includes(".png") || keyLower.endsWith(".png")) {
          mimeType = "image/png";
        } else if (
          keyLower.includes(".jpg") ||
          keyLower.endsWith(".jpg") ||
          keyLower.includes(".jpeg") ||
          keyLower.endsWith(".jpeg")
        ) {
          mimeType = "image/jpeg";
        } else if (keyLower.includes(".webp") || keyLower.endsWith(".webp")) {
          mimeType = "image/webp";
        } else if (keyLower.includes(".gif") || keyLower.endsWith(".gif")) {
          mimeType = "image/gif";
        } else if (keyLower.includes(".bmp") || keyLower.endsWith(".bmp")) {
          mimeType = "image/bmp";
        }
        ...
        logger.error(`Failed to download file from S3: ${errorMessage}`);
        await ErrorLogService.logServiceError(
          "GeminiService",
          "extractInvoiceData_S3Download_SystemError",
          error,
          { documentType, s3Key, mimeType, systemError: true }
        );
        throw new Error(
          `Cannot access the file at ${s3Key}. S3 access error: ${errorMessage}`
        );
      }
        ...
        try {
          ...
          if (axios.isAxiosError(error)) {
            ...
            const errorMessage =
              error instanceof Error ? error.message : String(error);
            logger.error(
              ` Model ${modelName} failed with non-HTTP error: ${errorMessage}`
            );
            await ErrorLogService.logServiceError(
              "GeminiService",
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to extract ${documentType} data: ${errorMessage}`
      );
    }
  }
