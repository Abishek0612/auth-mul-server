...
class DocumentService {
  async uploadDocument(
    ...
    if (documentType.toLowerCase() === "purchase order") {
      uploadedDoc = await PurchaseOrderService.uploadPurchaseOrder(
        file,
        fileName,
        fileType,
        userId,
      ...
      uploadedDoc = await GRNService.uploadGRN(
        file,
        fileName,
        fileType,
        userId,
      ...
      uploadedDoc = await PaymentAdviceService.uploadPaymentAdvice(
        file,
        fileName,
        fileType,
        userId,
      ...
      uploadedDoc = await InvoiceService.uploadInvoice(
        file,
        fileName,
        fileType,
        userId,
    ...
    return {
      ...uploadedDoc,
      s3_url: S3Service.getSignedUrl(uploadedDoc.s3_key),
    };
  }
  ...
  async getAllDocumentsByOrganization(
    ...
    try {
      ...
      if (filters?.status && filters.status !== "All Statuses") {
        invoiceQuery.status = filters.status;
        poQuery.status = filters.status;
        grnQuery.status = filters.status;
        paQuery.status = filters.status;
      }
      ...
      if (filters?.dateRange) {
        ...
        grnQuery.createdAt = dateFilter;
        paQuery.createdAt = dateFilter;
      }

      const [allInvoices, allPurchaseOrders, allGRNs, allPaymentAdvices] =
        await Promise.all([
      ...
      const transformedPurchaseOrders = allPurchaseOrders.map((po) => ({
        ...po,
        document_type: "Purchase Order",
      }));

      const transformedGRNs = allGRNs.map((grn) => ({
        ...grn,
        document_type: "GRN",
      }));

      const transformedPaymentAdvices = allPaymentAdvices.map((pa) => ({
        ...pa,
      ...
      if (filters?.type && filters.type !== "All Types") {
        allDocuments = allDocuments.filter(
          (doc) => doc.document_type === filters.type
        );
      }
      ...
      if (filters?.search) {
        ...
        allDocuments = allDocuments.filter((doc) => {
          ...
          const extractedData =
            doc.invoice_data ||
            doc.purchase_order_data ||
            doc.grn_data ||
            doc.payment_advice_data ||
            {};
  ...
  async getDocumentById(
    ...
    if (documentType.toLowerCase() === "purchase order") {
      ...
      document = {
        ...po,
        document_type: "Purchase Order",
        s3_url: S3Service.getSignedUrl(po.s3_key),
      };
    } else if (documentType.toLowerCase() === "grn") {
      ...
      document = {
        ...grn,
        document_type: "GRN",
        s3_url: S3Service.getSignedUrl(grn.s3_key),
      };
    } else if (documentType.toLowerCase() === "payment advice") {
      const paymentAdvice =
      ...
      document = {
        ...paymentAdvice,
        document_type: "Payment Advice",
        s3_url: S3Service.getSignedUrl(paymentAdvice.s3_key),
      };
    } else {
      ...
      document = {
        ...invoice,
        document_type: invoice.document_type || "Invoice",
        s3_url: S3Service.getSignedUrl(invoice.s3_key),
      };
    }
  ...
  async checkDocumentDuplicates(
    ...
    try {
      ...
      if (documentType.toLowerCase() === "purchase order") {
        currentDocument =
          await PurchaseOrderService.getPurchaseOrderWithOrganization(
            documentId
          );
      } else if (documentType.toLowerCase() === "grn") {
        currentDocument = await GRNService.getGRNWithOrganization(documentId);
      } else if (documentType.toLowerCase() === "payment advice") {
        currentDocument =
          await PaymentAdviceService.getPaymentAdviceWithOrganization(
            documentId
          );
      } else {
        currentDocument = await InvoiceService.getInvoiceWithOrganization(
          documentId
        );
      }
      ...
      throw new Error(
        `Failed to check document duplicates: ${
  ...
  private processDateFields(data: any, documentType: string): any {
    ...
    const fieldsToProcess =
      dateFields[documentType as keyof typeof dateFields] || [];

    fieldsToProcess.forEach((field: string) => {
  ...
  private getOrganizationFieldConfig(orgCode: string): any {
  ...
  async exportDocumentsToExcel(
    ...
    try {
      ...
      for (const doc of validDocuments) {
        ...
        docConfig.fields.forEach((field: any) => {
        ...
        if (extractedData.items && Array.isArray(extractedData.items)) {
          extractedData.items.forEach((item: any, index: number) => {
            ...
            docConfig.itemFields.forEach((field: any) => {
