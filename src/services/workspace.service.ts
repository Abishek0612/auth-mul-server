...
export class WorkspaceService {
  async getPOWorkspace(
    ...
    const allPOs = await purchaseorderModel.find(query).lean();
    ...
    const paginatedPOs = filteredPOs.slice(skip, skip + limit);

    return {
      documents: paginatedPOs,
      summary,
      totals,
      pagination: {
        total: totalCount,
  ...
  async getInvoiceWorkspace(
    ...
    const allInvoices = await invoiceModel.find(query).lean();
  ...
  async getGRNWorkspace(
    ...
    const allGRNs = await grnModel.find(query).lean();

    const enrichedGRNs = await Promise.all(
      allGRNs.map(async (grn) => {
        const grnData = grn.grn_data || {};
    ...
    if (filters.invoiceStatus && filters.invoiceStatus.length > 0) {
      filteredGRNs = filteredGRNs.filter((grn) =>
        filters.invoiceStatus!.includes(grn.invoiceStatus)
      );
    }

    if (filters.acceptanceStatus && filters.acceptanceStatus.length > 0) {
      filteredGRNs = filteredGRNs.filter((grn) =>
        filters.acceptanceStatus!.includes(grn.acceptanceStatus)
      );
    }
    ...
    const totalCount = filteredGRNs.length;
    const { page, limit } = pagination;
    const skip = (page - 1) * limit;
    const paginatedGRNs = filteredGRNs.slice(skip, skip + limit);

    return {
      documents: paginatedGRNs,
      summary,
      totals,
      pagination: {
        total: totalCount,
